...
Telefon CHAR(11) UNIQUE NOT NULL CHECK (Telefon NOT LIKE '%[^0-9]%'),
...
SELECT 
    ...
    SUM(S.SiparisTutari) AS Toplam_Ciro,
    AVG(S.SiparisTutari) AS Ortalama_Sepet_Tutari
FROM Restoranlar R
JOIN Siparisler S ON R.Restoran_ID = S.Restoran_ID
GROUP BY R.RestoranAdi
HAVING SUM(S.SiparisTutari) > 500;


-- Senaryo: Platformda kayýtlý olup henüz hiç "Askýda Yemek" baðýþý yapmamýþ kullanýcýlar

SELECT Kullanici_AdSoyad, Email, Rol
FROM Kullanicilar
WHERE Kullanici_ID NOT IN (
    SELECT DISTINCT Bagisci_ID 
    FROM Askida_Havuz 
    WHERE Bagisci_ID IS NOT NULL
);

-- VERÝTABANI PERFORMANS ÝYÝLEÞTÝRMELERÝ


-- 1. Ýndeks: Sipariþler tablosunda tarihe göre çok sýk arama/sýralama yapýlacaðý için
CREATE INDEX IDX_SiparisTarihi ON Siparisler(SiparisTarihi);

-- 2. Ýndeks: Restoranlar tablosunda isme göre arama yapýlmasý çok yaygýn bir iþlem olduðu için
